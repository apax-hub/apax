...
def test_mace_readout_vmapped_over_atoms():
    ...
    hidden_dim = 16
    n_atoms = 5
    readout = MaceReadout(num_interactions=num_interactions)
...
def test_mace_readout_derives_hidden_dim_from_input_width():
    """hidden_dim is not configured; it is read from the per-atom feature width
    (input_width // num_interactions), so it cannot desync from the descriptor."""
    num_interactions = 2
    hidden_dim = 8
    readout = MaceReadout(num_interactions=num_interactions)
    x = jnp.ones((num_interactions * hidden_dim,))
...
def test_mace_readout_rejects_indivisible_width():
    """A feature width not divisible by num_interactions is a layout mismatch
    and must fail loudly."""
    import pytest

    readout = MaceReadout(num_interactions=3)
    x = jnp.ones((16,))  # 16 is not divisible by 3
    with pytest.raises(ValueError):
        readout.init(jax.random.PRNGKey(0), x)
