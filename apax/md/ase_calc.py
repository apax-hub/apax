...
class ASECalculator(Calculator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        save_to_atoms:
            Save additional information generated by the model like energy
            uncertencies to be written into the atoms.info atribute.
        """
            ...
            if self.save_to_atoms is True: # Work only if bool True
                for k, v in self.results.items():
                    if isinstance(v, float):
                        ...
                        atoms.arrays[k] = v
            elif isinstance(self.save_to_atoms, list):
                try:
                    for k in self.save_to_atoms:
                        v = self.results[k]
                        if isinstance(v, float):
                            atoms.info[k] = v
                        else:
