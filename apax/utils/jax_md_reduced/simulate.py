...
def nvt_nose_hoover(
    ...
    constraint_idxs: Optional[list[int] | None] = None,
    **sim_kwargs,
) -> Simulator:
...
def npt_nose_hoover(
    ...
    def init_fn(key, R, box, mass=f32(1.0), **kwargs):
        ...
        state = initialize_momenta(state, key, _kT)
        KE = kinetic_energy(state)
        return state.set(thermostat=thermostat.initialize(quantity.count_dof(R), KE, _kT))
...
def hybrid_swap_mc(
    ...
    nbr_energy_fn = smap.pair_neighbor_list(wrapped_energy_fn, metric_fn, sigma=sigma_fn)
