...
def _diagonal_mask(X: Array) -> Array:
    ...
    if len(X.shape) > 3:
        raise ValueError(
            "Diagonal mask can only mask rank-2 or rank-3 tensors. Found {}.".format(
                len(X.shape)
            )
        )
    N = X.shape[0]
...
def _check_species_dtype(species):
    if species.dtype == i32 or species.dtype == i64:
        return
    msg = "Species has wrong dtype. Expected integer but found {}.".format(species.dtype)
...
def _vectorized_cond(pred: Array, fn: Callable[[Array], Array], operand: Array) -> Array:
...
def pair_neighbor_list(
    ...
    **kwargs,
    ...
    def fn_mapped(R: Array, neighbor: partition.NeighborList, **dynamic_kwargs) -> Array:
