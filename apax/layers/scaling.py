...
class MultiFidelityPerElementScaleShift(nn.Module):
    n_species: int = 119
    ...
    def setup(self):
        ...
        dtype = str_to_dtype(self.dtype)
        self.scale_param = self.param(
            "scale_per_element", scale_init, (n_species, self.num_fidelities, 1), dtype
        )
        self.shift_param = self.param(
            "shift_per_element", shift_init, (n_species, self.num_fidelities, 1), dtype
        )
